...
import com.gjmetal.star.kit.Kits;
import com.gjmetal.star.net.progress.ProgressHelper;
...
public class XApi {
    ...
    public Retrofit getRetrofit(String baseUrl, NetProvider provider, boolean useRx) {
        ...
        OkHttpClient okHttpClient = getClient(baseUrl, provider);
    ...
    private OkHttpClient getClient(String baseUrl, NetProvider provider) {
        ...
        if (provider.configGzip()) {
            builder.addInterceptor(new GzipRequestInterceptor());
        }

        OkHttpClient client = builder.build();
        //fix OkHttp调用HTTPS遇到的问题之：SSLPeerUnverifiedException
                ...
                return upstream.flatMap(new Function<T, Publisher<T>>() {
                    ...
                    public Publisher<T> apply(T model) {
                        if (model == null || model.isNull()) {
                            ...
                            return Flowable.error(new NetError(model.getErrorMsg(), model.getErrorCode()));//业务请求失败
                        } else {
