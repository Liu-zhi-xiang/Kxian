...
public class StrUtil {
     ...
     * @return
     */
    public static int getPriceBits(String price) {
        if (price == null || price.length() == 0||!price.contains(".")) {
            return 0;//为空默认保留0位
        }else {
            return price.substring(price.indexOf(".") + 1).length();
        }
    }
    ...
    public static String deleteEndZero(float f) {
        if (nf==null) {
    ...
    public static int[] getLemRightValueTwo(float height, float low) {
        int[] aa = new int[4];
        int count = 3;
        int c = 0;
        int scale = 0;
        int n = 3;
        int h = (int) Math.abs(height);
        int l = (int) Math.abs(low);
        if (height > 0 && low < 0) {
            if (Math.abs(height) > Math.abs(low)) {
                h = getLemMultipleMinimum(h, n);
                aa[0] = h;
    ...
    public static void main(String[] argc) {
        System.out.println(StrUtil.getAndOnePositiveNumber(-11.5));
    }
