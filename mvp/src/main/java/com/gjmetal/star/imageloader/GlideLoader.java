package com.gjmetal.star.imageloader;
...
public class GlideLoader implements ILoader {
    ...
    @Override
    public void loadCirNet(ImageView target, String url, Options options) {
        DrawableTypeRequest mDrawableTypeRequest = getRequestManager(target.getContext()).load(url);
        if (options == null) options = Options.defaultOptions();

        if (options.loadingResId != Options.RES_NONE) {
            mDrawableTypeRequest.placeholder(options.loadingResId);
        }
        if (options.loadErrorResId != Options.RES_NONE) {
            mDrawableTypeRequest.error(options.loadErrorResId);
        }

        wrapScaleType(mDrawableTypeRequest, options)
                .diskCacheStrategy(DiskCacheStrategy.SOURCE)
                .crossFade()
                .transform(new GlideCircleTransform(target.getContext()))
                .into(target);
    }
