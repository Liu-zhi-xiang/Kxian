...
import com.star.kchart.formatter.BigValueFormatter;
...
//                canvas.drawLine(lastX, getY(lastPoint.getAvgPrice()), curX, getY(curPoint.getAvgPrice()), mAvgPaint);
        ...
        if (isLongPress) {
            ...
            //画指示线的时间
            String text = DateUtil.getStringDateByLong(point.getDate().getTime(), 8);
...
//        strings.add("均价:" + point.getAvgPrice());
    ...
    private void drawValue(Canvas canvas, int index) {
        ...
        if (index >= 0 && index < mPoints.size()) {
            ...
            text = "均价:" + point.getAverage();
//            canvas.drawText(text, x, y, mAvgPaint);
            //成交量
            text = "VOL:" + mVolumeFormatter.formatString(point.getVolume());
...
//        画时间
        ...
        canvas.drawText(DateUtil.getStringDateByLong(mFirstStartTime.getTime(), 8), 0, y, mTextPaint);

        mTextPaint.setColor(ContextCompat.getColor(getContext(), R.color.chart_time));//结束时间
        canvas.drawText(DateUtil.getStringDateByLong(mSecondEndTime.getTime(), 8), mWidth - mTextPaint.measureText(DateUtil.getStringDateByLong(mSecondEndTime.getTime(), 8)), y, mTextPaint);

        mTextPaint.setColor(ContextCompat.getColor(getContext(), R.color.chart_press_xian));//长按时线条颜色
        //成交量
        canvas.drawText(mVolumeFormatter.formatString(mVolumeMax), 0, mHeight + baseLine, mTextPaint);
    }
